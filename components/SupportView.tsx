...
import * as SupportContent from '../support-content';
...
const SupportView: React.FC<SupportViewProps> = ({ lang: defaultLang, onBack }) => {
  ...
  useEffect(() => {
    const rawMarkdown = lang === 'vi' ? SupportContent.SUPPORT_VI : SupportContent.SUPPORT_EN;
  ...
  useEffect(() => {
    const langParam = searchParams.get('lang') as 'vi' | 'en' | null;
    if (!langParam || (langParam !== 'vi' && langParam !== 'en')) {
          ...
          <div className="flex bg-white dark:bg-white/5 p-1 rounded-[20px] border border-slate-200 dark:border-white/10 shadow-sm">
            <button
              ...
              className={`px-6 py-2 rounded-[16px] text-[13px] font-bold transition-all ${
                lang === 'vi' ? 'bg-slate-900 text-white dark:bg-[#00c4a7] dark:text-white shadow-md' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300'
              }`}
            >
              Tiếng Việt
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-[16px] text-[13px] font-bold transition-all ${
                lang === 'en' ? 'bg-slate-900 text-white dark:bg-[#00c4a7] dark:text-white shadow-md' : 'text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300'
              }`}
            >
              English
            </button>
